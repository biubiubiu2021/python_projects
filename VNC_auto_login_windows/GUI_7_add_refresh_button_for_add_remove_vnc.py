...
class VNCWindows(QWidget):
    ...
    def unlock(self, hwnd):
        ...
        password = "Teawithrabbit@345678" if self.parent().password_aaa.isChecked() else "Autoxpiv@1Autoxpiv@1"
        status = "Yes" if self.parent().status_yes.isChecked() else "No"
...
class Window(QWidget):
    ...
    def populate_layout(self):
        self.vnc_windows_widget = VNCWindows(self.vnc_windows)
        self.layout.addWidget(self.vnc_windows_widget)

        self.layout.addStretch(1)  # Add a stretch to the layout
    ...
    def refresh(self):
        # Remove the current VNCWindows widget, refresh button and stretch
        ...
        self.refresh_button = None
        self.layout.removeItem(self.layout.itemAt(self.layout.count()-1))  # remove the stretch

        # Re-scan for VNC instances
        self.vnc_windows = {}
