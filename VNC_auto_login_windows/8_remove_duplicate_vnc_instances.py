...
def enum_windows_callback(hwnd, vnc_windows):
    ...
    if "VNC Viewer" in window_text and not window_text.startswith("VNC Viewer"):
        ...
        # If the title_key is not in the dictionary, or the current hwnd is larger, add/update it
        if title_key not in vnc_windows or hwnd > vnc_windows[title_key][0]:
...
if 0 <= selection < len(vnc_windows):
    ...
    if username_filled == 'yes':
        login_to_windows_without_username(selected_hwnd, "Teawithrabbit@345678")
    else:
