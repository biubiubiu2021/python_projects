from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QLabel, QInputDialog, QMenu, QAction,QVBoxLayout,QMessageBox, QTextBrowser, QTextEdit, QCheckBox, QTableWidget, QTableWidgetItem, QHBoxLayout, QDialog
...
class Singleton:
    ...
    @classmethod
    def is_ingredient_saved(cls, ingredient):
        return any(line['Ingredient'] == ingredient for line in cls.selected_lines)

    @classmethod
    def get_line(cls, ingredient):
        return next((line for line in cls.selected_lines if line['Ingredient'] == ingredient), None)

    @classmethod
    def replace_line(cls, old_line, new_line):
        cls.selected_lines.remove(old_line)
        cls.selected_lines.append(new_line)


class ChoiceDialog(QDialog):
    def __init__(self, old_line, new_line):
        super().__init__()
        self.setWindowTitle("Choose a line to save")
        self.old_line = old_line
        self.new_line = new_line
        self.button1 = QPushButton(str(old_line), self)
        self.button1.clicked.connect(lambda: self.done(1))
        self.button2 = QPushButton(str(new_line), self)
        self.button2.clicked.connect(lambda: self.done(2))
    ...
    def done(self, result):
        if result == 1:
            print("User chose to keep the existing line.")
        elif result == 2:
            print("User chose to replace the line.")
            Singleton.replace_line(self.old_line, self.new_line)
        super().done(result)
...
class TableWindow2(QDialog):
    def __init__(self, data1, data2, data3, data4, data5):
        ...
        self.table1.setColumnCount(2)
        self.table1.setHorizontalHeaderLabels(["Ingredient", "Version"])
        self.selected_lines = []  # List to store selected lines
    ...
    def contextMenuEvent(self, event):
        menu = QMenu(self)
        save_action = QAction("Save Line to Dictionary", self)
        save_action.triggered.connect(self.saveLineToDictionary)
        menu.addAction(save_action)
        menu.exec_(event.globalPos())

    def saveLineToDictionary(self):
        focused_widget = QApplication.focusWidget()
        if isinstance(focused_widget, QTableWidget):
            selected_row = focused_widget.currentRow()
            if selected_row >= 0:
                ingredient_item = focused_widget.item(selected_row, 0)
                version_item = focused_widget.item(selected_row, 1)
                if ingredient_item and version_item:
                    ingredient = ingredient_item.text()
                    version = version_item.text()
                    kitID = focused_widget.item(0, 1).text()
                    line = {"KitID": kitID, "Ingredient": ingredient, "Version": version}

                    old_line = Singleton.get_line(ingredient)
                    if old_line:
                        dialog = ChoiceDialog(old_line, line)
                        dialog.exec_()
                    else:
                        Singleton.selected_lines.append(line)
                    print("Updated selected lines:", Singleton.selected_lines)
...
# add a button to display the select dictionary

        self.display_button = QPushButton("Display Dictionary", self)
        self.display_button.move(450, 10)  # Adjust the position as needed
        self.display_button.clicked.connect(self.displayDictionary)
    ...
    def saveLineToDictionary(self):
        ...
        if selected_row >= 0:
            ingredient_item = self.table.item(selected_row, 0)
            version_item = self.table.item(selected_row, 1)
            if ingredient_item and version_item:
                ingredient = ingredient_item.text()
                version = version_item.text()
                kitID = self.label2.text().split(">")[-2].split("<")[0]  # Extract the content between ">" and "<"
                line = {"KitID": kitID, "Ingredient": ingredient, "Version": version}
...
class DisplayDictionaryWindow(QDialog):
    def __init__(self, data):
        super().__init__()
        self.setWindowTitle("Display Dictionary")
        self.setFixedSize(500, 400)

        self.table = QTableWidget()
        self.table.setRowCount(len(data))
        self.table.setColumnCount(3)
        self.table.setHorizontalHeaderLabels(["KitID", "Ingredient", "Version"])

        for row, line in enumerate(data):
            item_kitID = QTableWidgetItem(line["KitID"])
            item_ingredient = QTableWidgetItem(line["Ingredient"])
            item_version = QTableWidgetItem(line["Version"])
            self.table.setItem(row, 0, item_kitID)
            self.table.setItem(row, 1, item_ingredient)
            self.table.setItem(row, 2, item_version)
        self.table.resizeColumnsToContents()
        self.table.resizeRowsToContents()
